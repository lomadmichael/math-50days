...
export default function YouTubePlayer({ lecture, onWatched }: YouTubePlayerProps) {
  ...
  if (!hasVideo && hasEbs) {
    return <EbsLinkCard url={lecture.ebsLectureUrl!} title={lecture.title} description={lecture.description} />;
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {hasEbs && <EbsSmallLink url={lecture.ebsLectureUrl!} />}
    </div>
  );
}
...
function EbsSmallLink({ url }: { url: string }) {
  return (
    <a
      href={url}
      target="_blank"
